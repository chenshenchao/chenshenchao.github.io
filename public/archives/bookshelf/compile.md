...
本书是从工具出发，通过 文法 来开发编程语言，非常适合入门编译原理。
ANTLR 的文法是可以不混入目标语言的，这样就不会文法里夹杂着 Java 或者 C#、Go 的代码，导致阅读麻烦。
示例也循序渐进，从简单的单词计数到 JSON 解析、到 SQL 解析、到自建脚本语言，完整地编译器入门流程。
基本学完你就能开发自己的脚本语言了。
...
这一本算是最好入门且实践的编译书籍，因为是从工具出发，且基于文法。
但是 yacc 的文法和 C 语言混在一起，会增加一些阅读的麻烦。
...
此书是松本行弘的博客编撰再增补而成的。
开篇就是很多关于语言设计的讨论，都是比较泛泛的语言特性讨论。
这本书适合有编程经验，且想开发编译器的编译器新手阅读，了解开发编译器和开发网站、APP的区别，了解大概开发编译器会遇到什么问题、要思考些什么问题。
本书只是个开始，是一些零散的博文，并非手把手的编译器开发书籍。
这本书会让你了解一个流行编程语言之父的思考，以及如何开发一个实用编程语言的心路历程。
本书的深度并不是很深，但是广度是有的，给想写编译器又不了解编译器开发的开发者的介绍书籍。
如果你是编译器新手，那么读完此书你编译器还入不了门，因为没有基础。但是可以打开你写编译器的大门。
如果你已经开发过解释器或者编译器了，可以从书中与作者做对比，从而发现自己自身的一些问题。
所以这是本适合各个阶段的人读的博客集。
...
本书在在讲词法时没有讲 DFA，反而在讲语法时讲了 DFA ，还用 DFA 来做语法分析，而 DFA 能力低于 LL(1)，是语法分析里面最弱的。
讲语法时没有讲 CFG 而是使用图来讲解，而图却也是画得不够明了清晰。
本书没有使用 yacc、antlr 这种编译器编译器，完全手工代码实现编译。